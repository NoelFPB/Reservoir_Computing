# No masks, full range mapping, and now testing not sending blank images
...
FEATURE_STORE = os.path.join("MNIST", "no_mask_full_swing_no_zeros.npz")
...
class PhotonicReservoirMNIST(PhotonicReservoir):
    ...
    def process_spatial_pattern(self, image_pixels):
        ...
        For each 7-pixel chunk (band):
            x in [0,1]  ->  v = V_MIN + (V_MAX - V_MIN) * x

        Bands that are (almost) blank are not sent to hardware; instead we reuse
        a pre-measured "blank band" PD response.
        """
        ...
        # -----------------------------
        # 1) Measure blank-band template once (all heaters at V_MIN)
        # -----------------------------
        if not hasattr(self, "blank_pd"):
            blank_v = [vmin] * chunk_size
            send(heaters, blank_v)
            blank_pd = read_many(avg=readavg)
            # store as numpy array template
            self.blank_pd = np.asarray(blank_pd, float)
            # print("Measured blank PD template:", self.blank_pd)

        # Threshold to decide "blank" band (sum of 7 pixels)
        # MNIST pixels are in [0,1], so for all-zero band sum=0.
        # You can tune this (e.g. 0.01, 0.05, etc.).
        ...
        for i in range(num_chunks):
            ...
            if np.sum(px) < blank_threshold:
                # Nearly blank band: reuse the blank PD response, no hardware call
                print('blank')
                features.append(self.blank_pd.copy())
                continue

            # Non-blank band: drive heaters according to pixel values
            v = vmin + scale * px          # 0 -> V_MIN, 1 -> V_MAX
    ...
    def MASK_process_spatial_pattern(self, image_pixels):
        K_req   = int(K_VIRTUAL)
        readavg = int(READ_AVG)
        vmin    = float(V_MIN)
        vmax    = float(V_MAX)
